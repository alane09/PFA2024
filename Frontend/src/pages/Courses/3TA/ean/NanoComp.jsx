    ...
    const getCourses = async () => {
        try {
            ...
            const filteredCourses = response.data.filter(course => (
                course.matiere === "nanocomp" && course.examen === 0
    ...
    const getExams = async () => {
        try {
            ...
            const filteredExams = response.data.filter(course => (
                course.matiere === "nanocomp" && course.examen === 1
            ));
        ...
        <body>
            <main>
                <div className='container'>
                    <h1 style={{ textAlign: 'center' }}><strong> Modélisation des microsystèmes</strong></h1>
                </div>
