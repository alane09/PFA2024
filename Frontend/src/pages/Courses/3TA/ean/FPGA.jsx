    ...
    const getCourses = async () => {
        try {
            ...
            const filteredCourses = response.data.filter(course => (
                course.matiere === "FPGA" && course.examen === 0
    ...
    const getExams = async () => {
        try {
            ...
            const filteredExams = response.data.filter(course => (
                course.matiere === "FPGA" && course.examen === 1
            ));
        ...
        <body>
            <main>
                <div className='container'>
                    <h1 style={{ textAlign: 'center' }}><strong> Architecture des systèmes reconfigurables (FPGA et SOC) </strong></h1>
                </div>
