    ...
    const getCourses = async () => {
        try {
            ...
            const filteredCourses = response.data.filter(course => (
                course.matiere === "BGD" && course.examen === 0
    ...
    const getExams = async () => {
        try {
            ...
            const filteredExams = response.data.filter(course => (
                course.matiere === "BGD" && course.examen === 1
            ));
        ...
        <body>
            <main>
                <div className='container'>
                    <h1 style={{ textAlign: 'center' }}><strong> Big data : gestion et analyse des données massives (BGD)</strong></h1>
                </div>
